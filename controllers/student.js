      ...
      TestItem.findOne({ where: { studentId: studentId, testId: testId } })
        .then((testItem) => {
          ...
          if (testItem) {
            ...
            testEndTime.setTime(
              testEndTime.getTime() + test.dataValues.timeAllotted * 60 * 1000
            );
...
exports.getTestList = (req, res, next) => {
  const studentId = 1;
  Performance.findAll({ where: { studentId: studentId } })
    .then((result) => {
      if (result.length > 0) {
...
exports.submitTest = (req, res, next) => {
  ...
  TestItem.findOne({
    where: { id: testItemId, studentId: studentId, testId: testId },
  }).then((result) => {
    if (!result) {
